...
**Last Updated:** 2026-04-22 (Phase 3b complete: VPC + cost guardrails)
...
- Husky + lint-staged pre-commit hooks (Oxlint + oxfmt)
...
- Terraform remote state backend in S3 with native locking (`terraform/bootstrap/`)
- Cost guardrails: AWS Budget at $5/mo + Cost Anomaly Detection, both with email alerts to the project owner (lives in `terraform/bootstrap/` so it keeps watching when the main config is destroyed)
- VPC + 9 subnets across 3 AZs (private/public/database tiers) + single managed NAT gateway + default-SG lockdown — provisioned via `terraform-aws-modules/vpc/aws` (`terraform/vpc.tf`, `terraform/security.tf`)
...
Module 02 is complete. Module 03 (Cloud Infrastructure & Terraform) started 2026-04-21. Phase 3a (remote state backend) and Phase 3b (VPC + cost guardrails) are complete; Phase 3c (EKS), Phase 3d (RDS), and the deploy-to-cloud checklist items remain. See [Module 03](./modules/module-03-cloud-infrastructure.md) for the in-progress checklist and per-phase Notes entries.

**Bursty-by-design committed (2026-04-22):** Cloud infra is now treated as ephemeral — created at session start, destroyed at session end, with always-on cost guardrails and (in 3c) a `Makefile` lifecycle harness. See [ADR-0012](./adr/0012-bursty-by-design-infra.md).
